...
def daily_plot_w_SMPS(
    ...
    """
    Create a daily plot with SMPS data.

    Parameters:
        dt (pd.DataFrame): DataFrame containing the ASOS data.
        dt_smps (SMPSDataset): SMPS dataset.
        plot_date (str): Date to plot (format: "YYYY-MM-DD").
        output_dir (str): Directory to save the plots.
        SMPS_plotting_timezone (str): Timezone for SMPS data.
        dt_precip (pd.DataFrame): DataFrame containing precipitation
            data, if you want to use a different dataset for precipitation.
    ...
    # Save the figure
    fname = f"PABI_daily_w_SMPS_{plot_date}.png"
    if output_dir:
...
def daily_plot(
    ...
    """
    THIS FUNCTIONS IS NOT COMPLETE YET, PLEASE DO NOT CALL THIS FUNCTION
    
    Create a daily plot for ASOS data, without SMPS data.
    ...
    # Save the figure
    fname = f"PABI_daily_{plot_date}.png"
    if output_dir:
        os.makedirs(output_dir, exist_ok=True)
        save_path = os.path.join(output_dir, fname)
    else:
